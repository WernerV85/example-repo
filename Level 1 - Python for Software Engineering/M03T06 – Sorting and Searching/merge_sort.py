...
def merge_sort(items):
    ...
    #print("====> merge_sort items:", items)
    return(items)

# Defining get_value function to return the length of a string(integer value)
def get_value(item):
...
def merge(items, sections, temporary_storage):
    ...
    while left_index < first_section_end or right_index < second_section_end:
        ...
        if left_index < first_section_end and right_index < second_section_end:
            ...
            if  get_value(items[left_index]) < get_value(items[right_index]):
